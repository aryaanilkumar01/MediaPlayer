...
function VideoCard({video,setDeleteVideoResponse,insideCategory}) {
  ...
  return (
    <>
      ...
      <Card.Body>
        <Card.Title className='d-flex justify-content-between align-items-center'>
          <h6 style={{height:'22px'}}>{video?.caption}</h6>
          {insideCategory?null:<button onClick={()=>removeVideo(video?.id)} className="btn"><i style={{height:'18px'}}
           className="fa-solid fa-trash text-danger"></i></button>}
        </Card.Title>
    ...
    <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
        <iframe className='w-100' height='387' src={`${video?.link}?autoplay=1`} title={video?.caption} 
