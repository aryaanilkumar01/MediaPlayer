...
import { addCategoryAPI,getAllCategoryAPI, getAVideoAPI, removeCategoryAPI, updateCategoryAPI } from '../services/allAPI';
...
function Category({dropResponse}) {
  ...
  const dragOver =(e)=>{
    console.log("video card draging over the caregory");
    e.preventDefault()
  }
  ...
  return (
    ...
    <div className='d-grid'>
      <Button className='btn btn-info' onClick={handleShow}>Add New Category
      ...
      
      {
        allCategories?.length>0?allCategories.map(category=>(
          <div className="border rounded p-3 mt-2" droppable="true" onDragOver={e=>dragOver(e)} onDrop={e=>videoDrop(e,category?.id)}>
            <div  onClick={() => setOpen(!open)} className="d-flex justify-content-between align-items-center">
              ...
              <button onClick={()=>removeCategory(category?.id)} className='btn'><i style={{height:'17px'}} className="fa-solid fa-trash text-danger"></i></button>
            </div>
           <Collapse in={open}>
              <Row>
                {
        ...
        )):<p className='fw-bolder fs-5 text-warning t-2'>No Categories are added yet!!!</p>
      }
